          ...
          <tr>
            <td style="background:#f9f9f9;padding:20px 32px;border-top:1px solid #eeeeee;">
              <p style="margin:0;color:#999999;font-size:12px;text-align:center;">
                © ${new Date().getFullYear()} Grihanivas. All rights reserved.<br/>
...
const leadNotificationTemplate = (lead) => {
  ...
  return baseTemplate(
    ...
    <p style="margin:0 0 24px;color:#666666;font-size:14px;">
      A new enquiry has been submitted on Grihanivas.
    </p>
...
const leadConfirmationTemplate = (lead) =>
  baseTemplate(
    'We received your enquiry — Grihanivas',
    `
...
const contactNotificationTemplate = (contact) =>
  baseTemplate(
    'New Contact Message — Grihanivas',
    `
...
const propertySubmissionNotificationTemplate = (sub) => {
  ...
  return baseTemplate(
    'New Property Listing — Grihanivas',
    `
    <h2 style="margin:0 0 8px;color:#1a1a1a;font-size:20px;">🏠 New Property Listing Submitted</h2>
    <p style="margin:0 0 24px;color:#666666;font-size:14px;">
      A user has submitted a property for review on Grihanivas.
    </p>
...
const sendLeadConfirmation = async (lead) => {
  ...
  return sendEmail({
    to: lead.email,
    subject: 'We received your enquiry — Grihanivas',
...
const sendWelcome = async (user) => {
  ...
  return sendEmail({
    to: user.email,
    subject: 'Welcome to Grihanivas 🎉',
    html: welcomeTemplate(user),
    text: `Welcome to Grihanivas! Your account is ready.`,
  });
};
...
const sendOtpEmail = async (email, otp) => {
  return sendEmail({
    to: email,
    subject: `${otp} is your Grihanivas OTP`,
