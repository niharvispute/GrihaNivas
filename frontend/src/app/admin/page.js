    ...
    <div className="space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-4">
        <div>
          ...
          <p className="text-slate-500 font-bold mt-2 font-body uppercase tracking-[0.2em] text-[10px]">Mumbai Editorial Management Dashboard</p>
        </div>
        <div className="flex items-center gap-4 bg-white border border-slate-100 rounded-3xl px-6 py-4 shadow-xl shadow-slate-200/50 cursor-pointer hover:border-primary transition-all group">
          ...
          <span className="text-sm font-black text-slate-900 uppercase tracking-widest">Oct 2024 (Active)</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 bg-white rounded-[3rem] border border-slate-50 shadow-sm overflow-hidden flex flex-col group hover:shadow-2xl transition-all">
          <div className="px-10 py-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/30 font-headline">
            <h2 className="text-lg font-black text-slate-900 tracking-widest uppercase">Leads Analytics</h2>
        ...
        <div className="bg-primary rounded-[3rem] p-10 text-white relative overflow-hidden flex flex-col justify-between shadow-2xl shadow-primary/30 group hover:scale-[1.02] transition-all">
          <div className="relative z-10 font-headline">
            <span className="text-[10px] font-black uppercase tracking-[0.3em] bg-white/20 px-4 py-2 rounded-full mb-10 inline-block">Flash Insight</span>
            <h2 className="text-4xl font-black leading-tight mb-4 tracking-tighter">Lodha World View</h2>
            <p className="text-white/70 text-lg font-medium mb-10 leading-relaxed">Currently trending as the most viewed property in MMR with 24 active leads.</p>
            <div className="flex -space-x-3 mb-10">
              {[1, 2, 3].map(i => (
                <img 
                  key={i} 
                  className="w-10 h-10 rounded-full border-4 border-primary bg-slate-200" 
      ...
      <div className="bg-white rounded-[3rem] border border-slate-50 shadow-sm overflow-hidden hover:shadow-2xl transition-all">
        <div className="px-10 py-10 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-50/30 font-headline">
          <div>
            <h2 className="text-lg font-black text-slate-900 uppercase tracking-widest">Incoming Submissions</h2>
            <p className="text-slate-400 text-sm font-bold mt-1 tracking-tight">Real-time engagement across the dashboard</p>
          </div>
        ...
        <div className="overflow-x-auto px-6 pb-6 mt-6">
          <table className="w-full text-left border-separate border-spacing-y-4 font-body">
            <thead>
            ...
            <tbody className="space-y-4">
              {recentLeads.map((lead, i) => (
                <tr key={i} className="bg-slate-50/50 hover:bg-white hover:shadow-xl transition-all group rounded-3xl">
                  ...
                  <td className="px-6 py-6 text-sm font-bold text-slate-500 uppercase tracking-widest text-[10px]">{lead.type}</td>
                  ...
                  <td className="px-6 py-6 text-right rounded-r-3xl">
                    <button className="w-10 h-10 flex items-center justify-center rounded-xl text-slate-300 hover:text-primary hover:bg-primary/5 transition-all">
                      <span className="material-symbols-outlined">shortcut</span>
                    </button>
                  </td>
                </tr>
              ))}
      ...
      {/* ➕ Global Action FAB */}
      <button className="fixed bottom-12 right-12 w-20 h-20 bg-slate-950 text-white rounded-[2.8rem] shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-50 group hover:rotate-90">
