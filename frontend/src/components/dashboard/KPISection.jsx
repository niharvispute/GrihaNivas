      ...
      {kpis.map((kpi, idx) => (
        <div key={idx} className="bg-white p-3 sm:p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between group hover:shadow-lg hover:shadow-slate-200/80 transition-all duration-500">
          <div className="flex justify-between items-start mb-2 sm:mb-3">
            <div className={`p-1.5 sm:p-2 rounded-xl ${themeClasses[kpi.theme]} group-hover:scale-110 transition-transform`}>
              <span className="material-symbols-outlined text-sm sm:text-xl">{kpi.icon}</span>
            </div>
          ...
          <div>
            <p className="text-[8px] sm:text-xs font-bold text-slate-400 mb-0.5">{kpi.title}</p>
            <h3 className="text-lg sm:text-2xl font-heading font-black text-slate-900">{kpi.value}</h3>
          </div>
        </div>
      ))}
    </div>
