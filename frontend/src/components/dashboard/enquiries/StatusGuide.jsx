export default function StatusGuide() {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      {steps.map((step, idx) => (
        <div key={idx} className="bg-white/60 backdrop-blur-xl border border-white p-8 rounded-[2rem] shadow-sm hover:shadow-xl transition-all duration-500 group">
          ...
          <p className="text-[11px] font-medium text-slate-400 leading-relaxed uppercase tracking-wider">{step.desc}</p>
        </div>
      ))}
    </div>
  );
