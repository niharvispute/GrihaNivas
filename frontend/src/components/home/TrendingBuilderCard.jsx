...
export default function TrendingBuilderCard({ builder }) {
  ...
  return (
    <article className="group relative bg-white rounded-xl md:rounded-3xl overflow-hidden shadow-sm md:shadow-md hover:shadow-2xl transition-all duration-500 flex flex-col h-full italic">
      {/* Image Section */}
      <div className={`relative ${'h-32 sm:h-48 md:h-56'} overflow-hidden flex-none bg-gradient-to-br from-slate-100 to-slate-50`}>
        ...
        <div className="absolute top-2 sm:top-4 left-2 sm:left-4 right-2 sm:right-4 flex gap-1.5 flex-wrap">
          ...
          <span className="bg-white/95 backdrop-blur-sm px-2 sm:px-4 py-0.5 sm:py-1.5 rounded-full text-[8px] sm:text-[10px] font-black tracking-wider uppercase text-primary shadow-lg italic">Trending</span>
        </div>
      </div>
      ...
      <div className="p-3 sm:p-5 md:p-6 flex flex-col grow gap-2 sm:gap-4">
        {/* Header */}
        <div>
          <h3 className="text-[13px] sm:text-base md:text-lg font-black text-slate-900 leading-tight mb-0.5 line-clamp-1 uppercase italic">
            {builderName}
          </h3>
        ...
        <div className="flex items-center justify-between gap-2 py-2 sm:py-3 border-t border-slate-100 mt-1">
          <div>
            ...
            <p className="text-xs sm:text-base font-black text-slate-900 italic">{established}</p>
          </div>
          <div className="border-l border-slate-100 pl-2 sm:pl-3">
            ...
            <p className="text-xs sm:text-base font-black text-primary italic">{projectCount}+</p>
          </div>
        </div>

        {/* Description */}
        <p className="text-slate-600 text-[10px] sm:text-xs line-clamp-1 sm:line-clamp-2 font-bold leading-relaxed italic flex-grow">
          {tagline}
        </p>
        ...
        <div className="flex gap-2.5 mt-auto pt-1">
          <Link
            href={detailHref}
            className="flex-1 bg-gradient-to-r from-primary to-primary/85 text-white py-2 sm:py-3 rounded-lg sm:rounded-xl font-black tracking-tighter hover:shadow-lg hover:shadow-primary/40 transition-all text-center text-[9px] sm:text-xs uppercase text-nowrap h-9 sm:h-11 flex items-center justify-center italic"
          >
            Details
          </Link>
          <Link
            href={`${detailHref}#portfolio`}
            className="flex-1 border-2 border-slate-200 text-slate-700 py-2 sm:py-3 rounded-lg sm:rounded-xl font-black tracking-tighter hover:border-primary hover:text-primary hover:bg-primary/5 transition-all text-center text-[9px] sm:text-xs uppercase text-nowrap h-9 sm:h-11 flex items-center justify-center hidden sm:flex italic"
          >
            Portfolio
          </Link>
        </div>
